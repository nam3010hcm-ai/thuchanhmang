...
import { ZoomIn, ZoomOut, Camera, FileJson, FileUp, Maximize, X } from 'lucide-react';
...
export default function NetworkLab() {
  ...
  const [rectSelect, setRectSelect] = useState<{ startX: number; startY: number; x: number; y: number; w: number; h: number } | null>(null);
  ...
  // Delete Confirmation State (hỗ trợ cả xóa 1 lẫn xóa nhiều thiết bị)
  ...
  useEffect(() => {
    ...
    if (saved && (saved.devices.length > 0 || saved.connections.length > 0)) {
      ...
      notify('Đã khôi phục sơ đồ từ phiên làm việc trước.', 'info');
    }
    setHasRestored(true);
  ...
  const addDevice = (type: DeviceType) => {
    ...
    notify(`Đã thêm ${newDevice.name}`, 'info');
  };
  ...
  const tryAssignDhcpForClients = (clientIds: string[], updatedConnections: Connection[]) => {
    clientIds.forEach(clientId => {
      ...
      if (assigned) {
        ...
        notify(`${client.name} nhận IP ${assigned.ip}/${assigned.mask} tự động từ DHCP (${router.name})`, 'success');
      }
    });
  };
  ...
  const finalizeDeleteDevices = () => {
    ...
    setSelectedDeviceIds(prev => prev.filter(id => !idSet.has(id)));
    notify(ids.length > 1 ? `Đã xóa ${ids.length} thiết bị` : 'Đã xóa thiết bị', 'error');
  ...
  /** Toạ độ con trỏ trong hệ toạ độ của canvas (đã trừ zoom), dùng cho kéo-thả và rubber-band select. */
  const getWorkspacePoint = (clientX: number, clientY: number) => {
    const rect = workspaceRef.current!.getBoundingClientRect();
    return { x: (clientX - rect.left + workspaceRef.current!.scrollLeft) / zoom, y: (clientY - rect.top + workspaceRef.current!.scrollTop) / zoom };
  };
  ...
  const handleDeviceDown = (e: React.PointerEvent, id: string) => {
    ...
    if (mode === ToolMode.CABLE) {
      if (cableStart === null) {
        setCableStart(id);
        notify('Chọn thiết bị đích...', 'info');
      } else {
        if (cableStart === id) {
          setCableStart(null);
          return;
        ...
        if (isSourceWireless || isTargetWireless) {
          if (!hasAPInvolved) {
            notify('Thiết bị di động chỉ có thể kết nối không dây với Access Point!', 'error');
            setCableStart(null);
            return;
          }
          setWifiAuthModal({
          ...
          if (sourceDev.ports !== 99 && sourceWiredCount >= sourceDev.ports) {
            notify(`${sourceDev.name} đã hết cổng LAN vật lý!`, 'error');
            setCableStart(null);
            return;
          }
          if (targetDev.ports !== 99 && targetWiredCount >= targetDev.ports) {
            notify(`${targetDev.name} đã hết cổng LAN vật lý!`, 'error');
            setCableStart(null);
            return;
          }
        }
        ...
        notify('Cắm dây mạng thành công!', 'success');
      }
      return;
    }

    if (mode === ToolMode.MOVE) {
      if (renamingDeviceId && renamingDeviceId !== id) setRenamingDeviceId(null);

      // Shift/Ctrl + click: chỉ bật/tắt trong danh sách chọn, không kéo
      if (e.shiftKey || e.ctrlKey || e.metaKey) {
        setSelectedDeviceIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
        return;
      }

      let activeSelection = selectedDeviceIds;
      if (!selectedDeviceIds.includes(id)) {
        activeSelection = [id];
        setSelectedDeviceIds(activeSelection);
      }

      pushHistory();
      setDraggedDevice(id);
      const startPositions = new Map<string, { x: number; y: number }>();
      activeSelection.forEach(did => {
        const d = devices.find(x => x.id === did);
        if (d) startPositions.set(did, { x: d.x, y: d.y });
      });
      setDragStartPositions(startPositions);
      setDragStartPointer(getWorkspacePoint(e.clientX, e.clientY));
    }
  };
  ...
  const handleWifiAuth = () => {
    ...
    if (wifiAuthModal.ssid === ap.ssid && wifiAuthModal.password === ap.password) {
      ...
      pushHistory();
      setConnections(updatedConnections);
      notify(`Đã kết nối thành công tới ${ap.name}`, 'success');
      ...
      notify('SSID hoặc Mật khẩu Wifi không chính xác!', 'error');
    }
  };
  ...
  const handleDeleteConnection = (e: React.MouseEvent, connectionId: string) => {
    if (mode === ToolMode.DELETE) {
      ...
      notify('Đã xóa cáp nối', 'error');
    }
  };

  // --- Bắt đầu rubber-band select khi nhấn vào vùng trống của canvas (chỉ ở chế độ Di chuyển) ---
  const handleWorkspacePointerDown = (e: React.PointerEvent) => {
    if (mode !== ToolMode.MOVE) return;
    if (e.target !== e.currentTarget && !(e.target as HTMLElement).classList.contains('canvas-scale-layer')) return;
    const pt = getWorkspacePoint(e.clientX, e.clientY);
    setRectSelect({ startX: pt.x, startY: pt.y, x: pt.x, y: pt.y, w: 0, h: 0 });
    if (!e.shiftKey && !e.ctrlKey && !e.metaKey) setSelectedDeviceIds([]);
    setRenamingDeviceId(null);
  };

  const handleWorkspacePointerMove = (e: React.PointerEvent) => {
    if (draggedDevice && mode === ToolMode.MOVE && dragStartPointer) {
      const pt = getWorkspacePoint(e.clientX, e.clientY);
    ...
    if (rectSelect) {
      const pt = getWorkspacePoint(e.clientX, e.clientY);
      const x = Math.min(rectSelect.startX, pt.x);
      const y = Math.min(rectSelect.startY, pt.y);
      const w = Math.abs(pt.x - rectSelect.startX);
      const h = Math.abs(pt.y - rectSelect.startY);
      setRectSelect({ ...rectSelect, x, y, w, h });

      const inRect = devices.filter(d => d.x + 64 >= x && d.x <= x + w && d.y + 64 >= y && d.y <= y + h).map(d => d.id);
      setSelectedDeviceIds(prev => {
        const base = (e.shiftKey || e.ctrlKey || e.metaKey) ? prev.filter(id => !inRect.includes(id)) : [];
        return Array.from(new Set([...base, ...inRect]));
      });
    }
  };
  ...
  // --- Ctrl/Cmd + cuộn chuột để zoom (dùng listener DOM gốc để preventDefault chặn zoom trình duyệt) ---
  // Phụ thuộc vào `hasRestored`: ở lần render đầu tiên (đang khôi phục localStorage) component trả về
  // null nên workspaceRef.current vẫn là null; phải gắn lại listener sau khi DOM thật được mount.
  useEffect(() => {
  ...
  const fitToScreen = () => {
    if (devices.length === 0 || !workspaceRef.current) {
      notify('Chưa có thiết bị nào trong lab.', 'info');
      return;
    }
    const margin = 100;
    const minX = Math.min(...devices.map(d => d.x)) - margin;
    const maxX = Math.max(...devices.map(d => d.x)) + margin + 100;
    const minY = Math.min(...devices.map(d => d.y)) - margin;
    const maxY = Math.max(...devices.map(d => d.y)) + margin + 100;
    const bboxW = maxX - minX;
    const bboxH = maxY - minY;
    const viewport = workspaceRef.current.getBoundingClientRect();
    const targetZoom = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, Math.min(viewport.width / bboxW, viewport.height / bboxH, 1.5)));
    setZoom(+targetZoom.toFixed(2));
    requestAnimationFrame(() => {
      if (!workspaceRef.current) return;
      const centerX = (minX + maxX) / 2;
      const centerY = (minY + maxY) / 2;
      workspaceRef.current.scrollLeft = Math.max(0, centerX * targetZoom - viewport.width / 2);
      workspaceRef.current.scrollTop = Math.max(0, centerY * targetZoom - viewport.height / 2);
    });
  };
  ...
  const runPing = () => {
    ...
    const source = devices.find(d => d.id === selectedDeviceIds[0]);
    if (!source) return;
  ...
  const saveAsImage = async () => {
    ...
    try {
      ...
      link.href = dataUrl;
      link.click();
      notify('Đã lưu ảnh sơ đồ thành công!', 'success');
    } catch (err) {
      console.error(err);
      notify('Lỗi khi xuất ảnh sơ đồ.', 'error');
    } finally {
  ...
  const resetLab = () => {
    ...
    clearPersistedLab();
    resetHistory();
    notify('Đã làm mới phòng lab', 'info');
  };
  ...
  const commitRename = (id: string, newName: string) => {
    ...
    notify(`Đã đổi tên thành "${newName}"`, 'info');
  };
  ...
  const loadTemplate = (template: TopologyTemplate) => {
    ...
    setSelectedDeviceIds([]);
    notify(`Đã tải sơ đồ mẫu: ${template.name}`, 'info');
  };
  ...
  const exportLabAsJson = () => {
    ...
    notify('Đã xuất sơ đồ ra file .json', 'success');
  };
  ...
  const handleImportLabFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try {
        ...
        setSelectedDeviceIds([]);
        notify('Đã nhập sơ đồ từ file .json', 'success');
      } catch (err) {
        notify('File .json không đúng định dạng sơ đồ NetLab.', 'error');
      }
    };
  ...
  const exportExerciseResult = () => {
    ...
    if (!exercise) {
      notify('Chưa chọn Đề bài để nộp.', 'error');
      return;
    }
    ...
    link.click();
    URL.revokeObjectURL(url);
    notify(`Đã xuất bài nộp (${report.passedCount}/${report.totalCount} tiêu chí đạt)`, 'success');
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.key === 'Delete' || e.key === 'Backspace') && selectedDeviceIds.length > 0) {
        setDeleteConfirmIds(selectedDeviceIds);
      }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [undo, redo, selectedDeviceIds]);
  ...
  const currentSelected = selectedDeviceIds.length === 1 ? devices.find(d => d.id === selectedDeviceIds[0]) : undefined;
  const isInternetConnected = currentSelected ? checkConnectionToType(currentSelected.id, 'isp', devices, connections) : false;
  const devicesToDelete = deleteConfirmIds ? devices.filter(d => deleteConfirmIds.includes(d.id)) : [];
      ...
      <main className="flex-1 flex flex-col relative bg-white">
        <div className="absolute top-4 left-4 right-4 z-10 flex justify-between items-center pointer-events-none">
          <div className="flex items-center gap-2 pointer-events-auto">
            <div className="bg-white/80 backdrop-blur px-4 py-2 rounded-full border border-slate-300 text-[10px] font-bold text-slate-500 flex items-center gap-4 shadow-2xl">
            ...
            {selectedDeviceIds.length > 1 && (
              <div className="bg-blue-600 text-white px-3 py-2 rounded-full text-[10px] font-bold flex items-center gap-2 shadow-2xl animate-in fade-in slide-in-from-left-2">
                <span>Đã chọn {selectedDeviceIds.length} thiết bị</span>
                <button onClick={() => setDeleteConfirmIds(selectedDeviceIds)} className="bg-white/20 hover:bg-white/30 rounded-full px-2 py-0.5 transition-colors">Xóa</button>
                <button onClick={() => setSelectedDeviceIds([])} className="hover:bg-white/20 rounded-full p-0.5 transition-colors"><X size={12} /></button>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2 pointer-events-auto">
            ...
            <button onClick={fitToScreen} title="Vừa khung hình" className="p-2 bg-slate-100 hover:bg-slate-200 rounded-lg border border-slate-300 shadow-xl transition-all active:scale-90"><Maximize size={16}/></button>
            <button onClick={() => setZoom(z => Math.max(ZOOM_MIN, +(z - 0.1).toFixed(2)))} className="p-2 bg-slate-100 hover:bg-slate-200 rounded-lg border border-slate-300 shadow-xl transition-all active:scale-90"><ZoomOut size={16}/></button>
            <div className="bg-slate-100 px-3 flex items-center text-xs font-bold rounded-lg border border-slate-300 shadow-xl" title="Giữ Ctrl + cuộn chuột để zoom">{Math.round(zoom * 100)}%</div>
            <button onClick={() => setZoom(z => Math.min(ZOOM_MAX, +(z + 0.1).toFixed(2)))} className="p-2 bg-slate-100 hover:bg-slate-200 rounded-lg border border-slate-300 shadow-xl transition-all active:scale-90"><ZoomIn size={16}/></button>
          </div>
        </div>
        ...
        >
          <div className="canvas-scale-layer" style={{ transform: `scale(${zoom})`, transformOrigin: '0 0', width: '200%', height: '200%' }}>
            <svg className="absolute top-0 left-0 w-full h-full" style={{ pointerEvents: 'none' }}>
              ...
              {rectSelect && (
                <rect
                  x={rectSelect.x} y={rectSelect.y} width={rectSelect.w} height={rectSelect.h}
                  fill="rgba(59,130,246,0.12)" stroke="#3b82f6" strokeWidth="1" strokeDasharray="4,3"
            ...
            {devices.map(device => (
              <DeviceNode
                ...
                isEditingName={renamingDeviceId === device.id}
                onPointerDown={handleDeviceDown}
                onNameDoubleClick={setRenamingDeviceId}
                onNameCommit={commitRename}
                onNameCancel={() => setRenamingDeviceId(null)}
              />
            ))}
          </div>
        </div>

        {deleteConfirmIds && devicesToDelete.length > 0 && (
          <DeleteConfirmModal
            devices={devicesToDelete}
            onCancel={() => setDeleteConfirmIds(null)}
            onConfirm={finalizeDeleteDevices}
          />
        )}

...
import {
  ...
  Terminal, Undo2, Redo2, LayoutTemplate, Zap
} from 'lucide-react';
import { Device, DeviceType, Connection, ToolMode, Exercise, ExerciseCheckResult } from '../types';
...
interface SidebarProps {
  ...
  exerciseResults: ExerciseCheckResult[] | null;
  onExportExerciseResult: () => void;
}
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  templates, onLoadTemplate, exercises, selectedExerciseId, onSelectExercise, exerciseResults, onExportExerciseResult
}) => {
  return (
    <aside className="w-72 bg-slate-100 border-r border-slate-300 flex flex-col z-20 shadow-xl overflow-y-auto shrink-0">
      <div className="p-4 border-b border-slate-300 bg-slate-100/70 sticky top-0 z-10 backdrop-blur">
        <div className="mb-4">
          ...
          <div className="pl-7 -mt-0.5">
            ...
            <p className="text-[9px] text-slate-500 font-medium">Bộ môn Toán - Tin học</p>
          </div>
        </div>
      ...
      <div className="p-4 space-y-6">
        <ExercisePanel
          ...
          onExportResult={onExportExerciseResult}
        />

        <section>
          <h2 className="text-[10px] font-bold text-slate-500 uppercase mb-3 tracking-widest flex items-center gap-1.5">
            <LayoutTemplate size={12} /> Thư viện sơ đồ mẫu
          </h2>
          <div className="space-y-2">
            {templates.map(t => (
              <button
                ...
                title={t.description}
                className="w-full text-left p-2.5 bg-slate-100 border border-slate-300 rounded-lg hover:bg-slate-200 transition-colors"
              >
                <div className="text-[11px] font-bold">{t.name}</div>
                <div className="text-[9px] text-slate-500 mt-0.5 line-clamp-2">{t.description}</div>
              </button>
            ))}
            <p className="text-[9px] text-slate-400 italic pt-1">Tải sơ đồ mẫu sẽ thay thế lab hiện tại (có thể Ctrl+Z để hoàn tác).</p>
          </div>
        </section>
            ...
            <div className="space-y-3">
              ...
              {currentSelected.ip !== undefined && (
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    <label className="block text-[9px] text-slate-500 font-bold mb-1 uppercase tracking-tight">SUBNET MASK</label>
                    <input
                      className="w-full bg-slate-100 border border-slate-300 rounded px-2 py-1 text-xs font-mono text-blue-700 focus:border-blue-500 outline-none"
                      value={currentSelected.mask || '255.255.255.0'}
                      onFocus={onFieldFocus}
                      onChange={(e) => onUpdateSelectedDevice({ mask: e.target.value })}
                    />
                  </div>
                </div>
              )}

              {currentSelected.type === 'router' && (
                <button
                  onClick={() => { onFieldFocus(); onUpdateSelectedDevice({ dhcpServerEnabled: !currentSelected.dhcpServerEnabled }); }}
                  className={`w-full flex items-center justify-between p-2 rounded-lg border text-[10px] font-bold transition-all ${
                    currentSelected.dhcpServerEnabled
                      ? 'bg-green-500/10 border-green-300 text-green-700'
                      : 'bg-slate-100 border-slate-300 text-slate-500'
                  }`}
                >
                  <span className="flex items-center gap-1.5"><Zap size={12} /> MÁY CHỦ DHCP</span>
                  <span className="uppercase px-2 py-0.5 rounded bg-white/60">{currentSelected.dhcpServerEnabled ? 'Bật' : 'Tắt'}</span>
                </button>
              )}

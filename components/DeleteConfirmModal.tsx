...
export const DeleteConfirmModal: React.FC<DeleteConfirmModalProps> = ({ devices, onCancel, onConfirm }) => {
        ...
        <div className="p-6 space-y-4 text-center">
          {isBulk ? (
            <p className="text-xs text-slate-700">
              Bạn có chắc chắn muốn xóa <span className="text-slate-900 font-bold">{devices.length} thiết bị</span> đã chọn?
              Mọi kết nối liên quan sẽ bị gỡ bỏ.
            </p>
          ) : (
            <p className="text-xs text-slate-700">
              Bạn có chắc chắn muốn xóa thiết bị <span className="text-slate-900 font-bold">{devices[0]?.name}</span>?
              Mọi kết nối liên quan sẽ bị gỡ bỏ.
            </p>
          )}

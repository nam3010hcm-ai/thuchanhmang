...
interface DeviceNodeProps {
  ...
  onNameDoubleClick: (id: string) => void;
  onNameCommit: (id: string, newName: string) => void;
  onNameCancel: () => void;
}

export const DeviceNode: React.FC<DeviceNodeProps> = ({
  device, isActive, isSelected, isCableStart, isDragging, wiredPortCount, isEditingName,
  onPointerDown, onNameDoubleClick, onNameCommit, onNameCancel
}) => {
  ...
  useEffect(() => {
    if (isEditingName) {
      ...
      // Đợi input render xong rồi mới focus + chọn toàn bộ text
      requestAnimationFrame(() => inputRef.current?.select());
    }
  }, [isEditingName, device.name]);

  const commit = () => {
    const trimmed = draftName.trim();
    onNameCommit(device.id, trimmed || device.name);
  };

  return (
    <div
      data-testid="device-node"
        ...
        {isEditingName ? (
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === 'Escape') { e.preventDefault(); onNameCancel(); }
            }}
            className="text-[10px] font-bold px-2 py-0.5 rounded border border-blue-500 outline-none w-20 text-center bg-white shadow-sm"
          />
        ) : (
          <div
            onDoubleClick={(e) => { e.stopPropagation(); onNameDoubleClick(device.id); }}
            title="Nhấp đúp để đổi tên"
            className="text-[10px] font-bold bg-white/90 px-2 py-0.5 rounded border border-slate-300 truncate max-w-[80px] shadow-sm cursor-text"
          >
            {device.name}
          </div>
        )}
